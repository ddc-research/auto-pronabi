...
```{r}
# documentos <- appsheet("documents")
documentos <- find_csv("documentos")

# users <- appsheet("users")
users <- find_csv("usuarios")

# vehiculos <- appsheet("vehicles")
vehiculos <- find_csv("vehiculos")
```
...
- Si tiene acta de devolución, remate, adjudicación, etc: Pendiente de cierre
...
estado_vehiculos |> 
	...
	labs(
		...
		caption = glue::glue("Total de vehículos: {nrow(vehiculos)}")
	)
```
